            ...
            if "data" in resp_json:
                ...
                if "sku_indicator_list" in data:
                    for sku in data['sku_indicator_list']:
                        ...
                        sku_exist = db_query_one(sku_exist_sql, {
                            "shop_id": sku['shop_id'],
                            "item_id": sku['item_id'],
                            "date": job_dict['date'],
                            "sku_id": sku['sku_id'],
                        ...
                        if sku_exist.empty:
                            db_insert("sku_indicator", sku)
                        else:
                            db_update("sku_indicator", sku, f"`id`={sku_exist['id']}")
                # 保留原始数据
                ...
                if exist.empty:
                    db_insert("item_overview", data)
                else:
                    db_update("item_overview", data, f"`id`={exist['id']}")

    def exec_handle(self):
        """ 监听任务列表 """
        watch_key = 'fordeal:item:overview:jobs'
        while True:
